...
const loginAdmin = asyncHandler(async (req, res) => {
  const { email, password } = req.body;
  const findAdmin = await Seller.findOne({ email })
...
const userCart = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (finduser == null) throw "not a user";
    } catch (err) {
      console.log(err);
      finduser = await Seller.findById(_id)
    }

    //* check if user already have products in cart
    const alreadyExistscart = await Cart.findOne({ orderby: finduser._id })
    if (alreadyExistscart) {
      for (let i = 0; i < alreadyExistscart.products.length; i++) {
        products.push(alreadyExistscart.products[i]);
      }
    }
...
const emptyCart = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(err);
      user = await Seller.findById(_id)
    }
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < userCart.products.length; i++) {
      let findProduct = await Product.find({ _id: userCart.products[i].product })
      console.log(findProduct)
      let newOrder = await new Order({
        products: userCart.products[i].product,
        paymentIntent: {
          id: uniqid(),
          method: "COD",
          amount: finalAmount,
        ...
        seller: findProduct[0].seller
      }).save()
    }

    // for different sellers, we can use the brand field of each product and create a custom order 
    // list for that brand
    // thus we need to create a function where we will pass in the order list
    // and then it will create orders brand by brand for a custmer
...
const getSellerOrders = asyncHandler(async (req, res) => { // Orders made by users to the seller
  const { brandname } = req.user;
  console.log(req.user);
  try {
    const userorders = await Order.find({ seller: brandname }).populate('products.product').exec()
    res.status(200).json({
      status: "success",
      message: "fetched all orders",
      data: {
